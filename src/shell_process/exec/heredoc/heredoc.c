...
/*   Updated: 2025/09/10 23:37:54 by sxrimu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int heredoc_writer_loop(int wfd, const char *raw_delim,
	int do_expand, t_data *data)
{
	char *lim;
	char *line;
	char *out;
	...
	while (1)
	{
		...
		if (write(wfd, out, ft_strlen(out)) < 0
			|| write(wfd, "\n", 1) < 0)
		{
			free(out);
			free(line);
...
static int build_heredoc_fd(t_redir *r, t_data *data)
{
	...
	if (pid == 0)
	{
		/* CHILD: défaut SIGINT/SIGQUIT (ton choix), readline fera interrompre le process */
		int rc;

		signals_setup_heredoc_child();
		close(p[0]);

		rc = heredoc_writer_loop(p[1], r->delim,
			r->delim_can_expand ? 1 : 0, data);
