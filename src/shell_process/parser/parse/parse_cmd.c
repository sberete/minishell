...
/*   Updated: 2025/09/16 21:32:39 by sberete          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_binop(t_token_type t)
{
	return (t == T_AND || t == T_OR || t == T_PIPE);
}
static int	can_start_cmd(t_token_type t)
{
	return (t == T_WORD || t == T_PAREN_OPEN || is_redir(t));
}
...
int	parser_syntax(const t_token *t)
{
	...
	paren = 0;
	t_token_type prev = T_SEPARATOR;
	while (t)
	{
		tt = t->type;
		if (tt == T_UNKNOWN)
			return (print_syntax_error("unknown token"), 0);
		if (tt == T_END)
			break ;
		if (tt == T_SEPARATOR)
		{
			if (prev == T_SEPARATOR || prev == T_PAREN_OPEN || is_binop(prev)
				|| is_redir(prev))
		...
		if (tt == T_PAREN_OPEN)
		{
			...
			prev = tt;
			t = t->next;
			if (!t || t->type == T_PAREN_CLOSE || t->type == T_SEPARATOR
				|| is_binop(t->type))
	...
	if (paren != 0)
		return (print_syntax_error("unclosed '('"), 0);
	if (prev == T_SEPARATOR || prev == T_PAREN_OPEN || is_binop(prev)
		|| is_redir(prev))
